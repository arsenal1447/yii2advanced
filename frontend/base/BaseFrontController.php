...
class BaseFrontController extends BaseController
{
    ...
    {
        $src=Ad::getWebUrl().'/images/forum.gif';
    ...
    private function getOneColumn($subBoards,$rowsCount)
    {
        ...
        for($i = 0; $i < $rowsCount; $i++)
        {
            ...
            {
                $href=Ad::getHomeUrl().'?r=thread/index&boardid='.$subBoard['id'];
                $target='_self';
                ...
                {
                    $td3 ='<a href="'.Ad::getHomeUrl().'?r=thread/view&id='.$subBoard['thread_id'].'"/>'.$subBoard['thread_title'].'</a>';
    ...
    public function buildBoards($boardId=0)
    {
        ...
        foreach ($boards as $board)
        {
            ...
            $html.='<div class="hd" style="border-bottom:none;"><h2><a href="'.Ad::getHomeUrl().'?r=board/index&boardid='.$id.'">'.$parentBoard['name'].'</a></h2></div>';
