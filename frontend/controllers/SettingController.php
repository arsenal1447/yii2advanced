...
class SettingController extends Controller
{
    ...
    {
        /** @var Donate $model */
        $model = AdDonate::findOne(['donate_user_id' => Yii::$app->user->id]) ?: new AdDonate(['scenario' => 'create']);
        $oldQrCode = $model->donate_qr_code;
        $model->donate_description ?: $model->donate_description = '如果这篇文章对您有帮助，不妨微信小额赞助我一下，让我有动力继续写出高质量的教程。';
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($image === false && !empty($oldQrCode)) {
                $model->donate_qr_code = $oldQrCode;
            }

            $model->donate_user_id = Yii::$app->user->id;
    ...
    public function actionDisconnect($id)
    {
        ...
        $account = UserAccount::findOne(['account_id' => $id]);
        if ($account === null) {
    ...
    public function connect(ClientInterface $client)
    {
        ...
        if ($account === null) {
            $account = Yii::createObject([
                ...
                'account_provider' => $provider,
                'account_client_id' => $clientId,
                'account_data' => json_encode($attributes),
                'account_user_id' => Yii::$app->user->id,
                'account_create' => time(),
            ]);
            $account->save(false);
