...
//         die('30');
    ...
    public function findUserActionNotify(UserMeta $meta)
    {
        if ($meta->meta_target_type == 'comment') {
            $condition['comment_id'] = $meta->meta_target_id;
        } else {
            $condition['post_id'] = $meta->meta_target_id;
        }
        return AdNotice::findOne([
                'from_user_id' => $meta->meta_user_id,
                'type' => $meta->meta_target_type . '_' . $meta->meta_type,
            ] + $condition);
    }
}